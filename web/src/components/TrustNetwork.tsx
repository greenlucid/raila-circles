  ...
  useEffect(() => {
    ...
    const loadNetwork = async () => {
      try {
        ...
        console.log('Trust relations:', relations)
        ...
        console.log('Mutual trust addresses:', uniqueAddresses)

        // Fetch profiles for each
        const connectionData = await Promise.all(
          uniqueAddresses.map(async (addr) => {
            try {
              // First get avatar to get CID
...
function ConnectionCard({ connection }: { connection: TrustConnection }) {
  const { data: hasModule } = useReadContract({
